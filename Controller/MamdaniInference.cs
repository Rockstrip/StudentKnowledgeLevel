    ...
    public static class MamdaniInference
    {
        ...
        public static IEnumerable<double[]> Implication(IEnumerable<(FuzzyTuple fuzzyTuple, Severity OutputSeverity)> rules)
        {
            ...
            foreach (var rule in rules)
            {
                ...
                var inputMfsMin = Math.Min(rule.fuzzyTuple.ApplicationGrade.Value, Math.Min(rule.fuzzyTuple.PresentationGrade.Value, rule.fuzzyTuple.ArrangementGrade.Value));
        ...
        private static IEnumerable<FuzzyTuple> GenerateFuzzyTuples(Dictionary<Severity, double>[] fuzzyValues)
        {
            ...
            return (from ml  in fuzzyValues[0] 
                    from cs1 in fuzzyValues[1] 
                    from cs2 in fuzzyValues[2] 
                    select new FuzzyTuple(ml, cs1, cs2)).ToList();
        }
    }
}
